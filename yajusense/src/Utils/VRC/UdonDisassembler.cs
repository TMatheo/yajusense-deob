...
{
	private static readonly string SaveDir = Path.Combine(Directory.GetCurrentDirectory(), "yajusense/UdonDisassembler");
	...
	private static EventInfo PrepareEventInformation(DisassemblyContext context)
	{
		...
		HashSet<string> eventNameFilter = result.FilterEvents ? new HashSet<string>(context.TargetEventNames, StringComparer.OrdinalIgnoreCase) : null;
		...
		if (result.FilterEvents)
		{
			...
			for (var i = 0; i < sortedAddresses.Count; i++)
			{
				...
				uint endAddr = i < sortedAddresses.Count - 1 ? sortedAddresses[i + 1] : (uint)context.UdonBehaviour._program.ByteCode.Length;
		...
		switch (opcode)
		{
			...
			case (uint)OpCode.PUSH:
				...
				if (heapValue != null && symbolName.Contains("_const_"))
				{
					...
					if (runtimeType == typeof(bool) || runtimeType == typeof(int) || runtimeType == typeof(float) || runtimeType == typeof(double))
						context.Constants[pushOffset] = heapValue.ToString();
					else if (runtimeType == typeof(string))
						context.Constants[pushOffset] = $"\"{heapValue}\"";
				}
	...
	private static void SaveDisassemblyOutput(DisassemblyContext context)
	{
		...
		if (context.Constants.Count > 0)
			SaveConstants(context, SaveDir, timestamp, filenameSuffix);

		string eventInfo = filtered ? $" (filtered to {context.EventAddresses.Count} events)" : "";
	...
	private static void SaveConstants(DisassemblyContext context, string saveDir, string timestamp, string filenameSuffix)
	{
		...
		foreach (KeyValuePair<uint, string> kv in context.Constants)
		{
	...
	{
		public UdonBehaviour UdonBehaviour { get; init; }
		public string UbName { get; init; }
		public IEnumerable<string> TargetEventNames { get; init; }
		public StringBuilder Output { get; init; }
		public Dictionary<uint, string> Constants { get; init; }
	...
	{
		public HashSet<uint> EventAddresses { get; init; }
		public Dictionary<uint, string> AddressToEventName { get; init; }
		public bool FilterEvents { get; init; }
		public Dictionary<uint, uint> EventBounds { get; init; }
	}
}